...
export const useCardsData = () => {
  ...
  const getTotals = async () => {
    try {
      ...
      const oneMonthBalance =
        data.oneMonthTotalIncome - data.oneMonthTotalExpense;

      const percentChange = (sum, oneMonthSum) => {
        if (oneMonthSum == 0) return 0;
        return (
          Math.round(((sum - oneMonthSum) / oneMonthSum) * 100 * 100) / 100
        );
      };
...
export const useLineGraphData = (type, value) => {
  ...
  useEffect(() => {
    if (type === "income") {
      if (value == "one") setGraphData(incomeData);
      else setGraphData(incomeData.slice(-6));
    } else if (type === "expense") {
      if (value == "one") setGraphData(expenseData);
      else setGraphData(expenseData.slice(-6));
    }
